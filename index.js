...
const invariantWords = new Set(["mais","pois","noix","riz","houmous"]);
...
function prepare(str) {
  return normalize(str).split(" ").map(singularize).join(" ");
}
...
const meats = [
  "poulet","boeuf","porc","dinde","veau","agneau","canard",
  "lard","saucisse","bacon","steak","epaule","jarret","lardon",
  "jambon","chorizo","gite de boeuf","jarret de boeuf","gite"
].map(prepare);

const fish = [
  "saumon","cabillaud","colin","truite","thon",
  "merlu","maquereau","sardine","crevette"
].map(prepare);

const dairy = [
  "lait","beurre","fromage","ricotta","parmesan","creme","yaourt",
  "mozzarella","chevre","comte","cream cheese","reblochon",
  "burrata","feta","cheddar"
].map(prepare);
...
app.get("/generate-week", (req, res) => {
  ...
  let meatCount = 0, fishCount = 0, vegCount = 0;
  let heavyCreamCount = 0, longCount = 0, veryLongCount = 0;
    ...
    let matchingRecipes = seasonal.filter(recipe =>
      recipe.ingredients.some(ing =>
        ingredientsToUse.some(userIng =>
          containsIngredient(prepare(ing.name), prepare(userIng))
        )
      )
    );
    matchingRecipes = matchingRecipes.filter(r => !lastGeneration.includes(r.id));
    if (matchingRecipes.length > 0) {
      matchingRecipes.sort((a, b) => a.lastUsed - b.lastUsed);
      const forced = shuffle(matchingRecipes.slice(0, 5))[0];
      addRecipe(forced);
  ...
  shuffle(meatRecipes).forEach(r => {
    if (meatCount < meatTarget && !selected.some(s => s.id === r.id)) addRecipe(r);
  });
  shuffle(fishRecipes).forEach(r => {
    if (fishCount < fishTarget && !selected.some(s => s.id === r.id)) addRecipe(r);
  });
  shuffle(vegetarianRecipes).forEach(r => {
    if (vegCount < vegetarianTarget && !selected.some(s => s.id === r.id)) addRecipe(r);
  });

  shuffle(seasonal).forEach(r => {
    if (selected.length >= 10) return;
    if (!selected.some(s => s.id === r.id) && canAdd(r)) addRecipe(r);
  });
  ...
  if (selected.length < 10) {
    const remainingPool = seasonal.filter(r => !selected.some(sel => sel.id === r.id));
    shuffle(remainingPool).forEach(r => {
      if (selected.length < 10 && canAdd(r)) addRecipe(r);
    });
  }
  ...
  selected = selected.map(recipe => {
    ...
    if (isVeryLong || isHard) adjustedServings = Math.max(recipe.servings, 4);
  ...
  selected.forEach(recipe => {
    recipe.ingredients.forEach(ingredient => {
      ...
      if (!shoppingList[category][ingredient.name]) {
        shoppingList[category][ingredient.name] = { quantity: 0, unit: ingredient.unit };
      }
  ...
  selected.forEach((sel, i) => {
    ...
    if (index !== -1) allRecipes[index].lastUsed = baseTime + (i * 1000);
  });
